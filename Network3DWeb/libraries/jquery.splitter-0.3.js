...
(function($, undefined) {
    ...
    $.fn.split = function(options) {
        ...
        var self = $.extend(this, {
                ...
                if (settings.orientation == 'vertical') {
                    return function(n) {
                        if (n === undefined) {
                        	...
                        	endRelativePosition = this.width()-n;
                            position = n;
                    ...
                    return function(n) {
                        if (n === undefined) {
                        	...
                        	endRelativePosition = this.height()-n;
            ...
            endRelativePosition:  (function() {
            	return function() {
                      return endRelativePosition;
                    };
            })(),
            orientation: settings.orientation,
            limit: settings.limit,
            relativeToEnd: settings.relativeToEnd,
            destroy: function() {
        ...
        self.bind('spliter.resize', function() {
            ...
            if (self.orientation == 'vertical'){
	            if(self.relativeToEnd){
		            pos = self.width()-self.endRelativePosition();
	            }
                if(pos > self.width()-self.limit){
                    pos = self.width()-self.limit;
                }else if (pos < self.limit){
                    pos = self.limit;
                }
            } else if (self.orientation == 'horizontal'){
	            if(self.relativeToEnd){
		            pos = self.height()-self.endRelativePosition();
	            }
                if(pos > self.height()-self.limit){
                    pos = self.height()-self.limit;
                }else if (pos < self.limit){
                    pos = self.limit;
                }
            }
        ...
        if(!settings.position){
	        ...
	        if(settings.position>=0){
	        	newposition = settings.position;
	        }else{
		        if (settings.orientation == 'vertical') {
			        newposition = width+settings.position;
			    } else if (settings.orientation == 'horizontal') {
			        newposition = height+settings.position;
			    }
		    }
        }
        position = newposition;
        self.position(position);
        this.find('.spliter_panel').trigger('spliter.resize');
            ...
            $(document.documentElement).bind('mousedown.spliter', function() {
                ...
                if (current_spliter !== null) {
                    ...
                    if (current_spliter.orientation == 'vertical') {
                        ...
                        if(x > current_spliter.width()-limit){
                            current_spliter.position(current_spliter.width()-limit);
                            current_spliter.find('.spliter_panel').trigger('spliter.resize');
                            return false;
                        }else if (x < current_spliter.limit){
                            current_spliter.position(current_spliter.limit);
                            current_spliter.find('.spliter_panel').trigger('spliter.resize');
                            return false;
                        }else{
                        ...
                        if(y>current_spliter.height()-limit){
                            current_spliter.position(current_spliter.height()-limit);
                            current_spliter.find('.spliter_panel').trigger('spliter.resize');
                            return false;
                        }else if(y < current_spliter.limit){
                            current_spliter.position(current_spliter.limit);
                            current_spliter.find('.spliter_panel').trigger('spliter.resize');
                            return false;
                        }else{
